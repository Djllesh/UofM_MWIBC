...
for i in range(np.size(phase, axis=2)):
    fig, ax = plt.subplots(
        ...
        2,
        sharex=True,
        **dict(figsize=(1400 / __MY_DPI, 1000 / __MY_DPI), dpi=__MY_DPI),
    )
    ...
    ax[0].plot(freqs, phase[1, :, i], "b--", linewidth=1.0, label="Wrapped")
    ax[0].axhline(
        y=unwrapped_phase[0], linestyle="--", color="black", linewidth=0.9
    )
    ...
    plt.savefig(
        os.path.join(
            "C://Users/prikh/Desktop/propagation_speed/", "antenna_pos_%d.png"
        )
        % i,
        dpi=__MY_DPI,
    )
