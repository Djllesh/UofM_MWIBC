...
if __name__ == "__main__":
    ...
    fig, ax = plt.subplots(
        ...
        sharex=True,
        sharey="row",
        **dict(figsize=(1920 / __MY_DPI, 1080 / __MY_DPI), dpi=__MY_DPI),
    )
    ...
    args = [
        (target_phase_unwrapped6, freqs, length),
    ...
    for idx, axis in enumerate(ax[0]):
        ...
        extracted_shape6 = phase_shape(
            freqs,
            length,
            epsilon=cole_cole(freqs, e_h6, e_s6, tau6, alpha6),
            shift=shift6,
        )
        extracted_shape5 = phase_shape(
            freqs,
            length,
            epsilon=cole_cole(freqs, e_h5, e_s5, tau5, alpha5),
            shift=shift5,
        )

        axis.plot(
            freqs,
            target_phase_unwrapped6,
            "k-",
            linewidth=1.5,
    ...
    for idx, axis in enumerate(ax[1]):
        ...
        experimental_speed5 = -2 * np.pi * length * freqs / exp_vals5

        experimental_speed6 = -2 * np.pi * length * freqs / exp_vals6
        ...
        axis.plot(
            ...
            "k-",
            linewidth=1.5,
            label="Experimental speed $6\cdot 2pi$ shift",
        )
        axis.plot(
            freqs,
            -2 * np.pi * length * freqs / extracted_shape6,
            "k--",
            linewidth=0.7,
            label="Theoretical speed $6\cdot 2pi$ shift",
        )
        axis.plot(
            freqs,
            experimental_speed5,
            "r-",
            linewidth=1.5,
            label="Experimental speed $5\cdot 2pi$ shift",
        )
        axis.plot(
            freqs,
            -2 * np.pi * length * freqs / extracted_shape5,
            "r--",
            linewidth=0.7,
            label="Theoretical speed $5\cdot 2pi$ shift",
        )
    ...
    # plt.show()
    fig.savefig(
        os.path.join(get_proj_path(), "output/cyl_phantom/unwrap_speed.png"),
        transparent=True,
    )
