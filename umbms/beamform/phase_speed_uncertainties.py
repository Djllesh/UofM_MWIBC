...
def d_eprime_e_s(freqs, tau, alpha):
    ...
    return (1 + (omega * tau) ** (1 - alpha) * np.sin(alpha * np.pi / 2)) / (
        1
        + 2 * (omega * tau) ** (1 - alpha) * np.sin(alpha * np.pi / 2)
        + (omega * tau) ** (2 * (1 - alpha))
    )
...
def d_eprime_e_h(freqs, tau, alpha):
    ...
    omega = 2 * np.pi * freqs
    return 1 - (
        (1 + (omega * tau) ** (1 - alpha) * np.sin(alpha * np.pi / 2))
        / (
            1
            + 2 * (omega * tau) ** (1 - alpha) * np.sin(alpha * np.pi / 2)
            + (omega * tau) ** (2 * (1 - alpha))
...
def d_eprime_alpha(e_h, e_s, freqs, tau, alpha):
    ...
    return (
        ...
        * (
            ...
            * tau
            * omega
            * np.cos(alpha * np.pi / 2)
            * (omega**2 * tau**2 - (omega * tau) ** (2 * alpha))
            + 2
            * np.log(omega * tau)
            * (
                omega
                ...
                * (3 * (omega * tau) ** (2 * alpha) - (omega * tau) ** 2)
                + 2 * (omega * tau) ** (3 * alpha)
            )
        )
    ) / (
        2
        * (
            2 * omega**3 * tau**3 * np.sin(alpha * np.pi / 2)
            + (omega * tau) ** (3 * alpha)
            + (omega * tau) ** (alpha + 2)
        )
        ** 2
...
def d_eprime_tau(e_h, e_s, freqs, tau, alpha):
    ...
    return (
        ...
        * (e_h - e_s)
        * (omega * tau) ** (alpha + 1)
        * (omega * tau * np.sin(alpha * np.pi / 2))
        * (3 * (omega * tau) ** (2 * alpha) - (omega * tau) ** 2)
        + 2 * (omega * tau) ** (3 * alpha)
    ) / (
...
def d_e2prime_e_h(freqs, tau, alpha):
    ...
    omega = 2 * np.pi * freqs
    return -(
        ((omega * tau) ** (1 - alpha) * np.cos(alpha * np.pi / 2))
        / (
            1
            + 2 * (omega * tau) ** (1 - alpha) * np.sin(alpha * np.pi / 2)
            + (omega * tau) ** (2 * (1 - alpha))
...
def deps_prime(
    freqs, e_h, e_s, tau, alpha, d_e_h, d_e_s, d_tau, d_alpha, d_shift
):
    """Returns the differential of the real part of the permittivity"""
    deps = np.sqrt(
        np.abs(d_eprime_e_s(freqs=freqs, tau=tau, alpha=alpha)) ** 2 * d_e_s**2
        + np.abs(d_eprime_e_h(freqs=freqs, tau=tau, alpha=alpha)) ** 2
        * d_e_h**2
        + np.abs(
            d_eprime_tau(freqs=freqs, e_h=e_h, e_s=e_s, tau=tau, alpha=alpha)
        )
        ** 2
        * d_tau**2
        + np.abs(
            d_eprime_alpha(freqs=freqs, e_h=e_h, e_s=e_s, tau=tau, alpha=alpha)
        )
        ** 2
        * d_alpha**2
        + d_shift**2
...
def deps_2prime(
    ...
    deps = np.sqrt(
        np.abs(d_e2prime_e_s(freqs=freqs, tau=tau, alpha=alpha)) ** 2 * d_e_s**2
        + np.abs(d_e2prime_e_h(freqs=freqs, tau=tau, alpha=alpha)) ** 2
        * d_e_h**2
        + np.abs(
            d_e2prime_tau(freqs=freqs, e_h=e_h, e_s=e_s, tau=tau, alpha=alpha)
        )
        ** 2
        * d_tau**2
        + np.abs(
            d_e2prime_alpha(freqs=freqs, e_h=e_h, e_s=e_s, tau=tau, alpha=alpha)
        )
        ** 2
        * d_alpha**2
        + d_shift**2
...
def dv_avg(
    ...
    dv_avg_ = np.sqrt(
        np.abs(2 * np.pi * freqs / phi) ** 2 * dL**2
