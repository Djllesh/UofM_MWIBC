...
from umbms.beamform.intersections_analytical import (
...
def get_pix_ts_old(
    ant_rad, m_size, roi_rad, speed, *, n_ant_pos=72, ini_ant_ang=-136.0
):
    """Get one-way pixel response times
    Parameters
    ----------
...
def get_circle_intersections_parallel(
    n_ant_pos,
    m_size,
    ant_xs,
    ant_ys,
...
def time_signal_per_antenna_modelled(
    tar_x,
    tar_y,
    tar_rad,
    ant_rad,
    ...
    *,
    ...
    ant_angs = np.flip(np.deg2rad(np.linspace(0, 355, n_ant_pos) + ant_ini_ang))
    ...
    if int_f_xs is None:
        for ant_pos in range(n_ant_pos):  # For every antenna
            distance = (
                np.sqrt(
                    (tar_x - ant_xs[ant_pos]) ** 2
                    + (tar_y - ant_ys[ant_pos]) ** 2
                )
                - tar_rad
            )
        ...
        for ant_pos in range(n_ant_pos):  # For every antenna
            ...
            plastic_to_target = (
                np.sqrt(
                    ...
                    + (int_f_ys[ant_pos, y_idx, x_idx] - tar_y) ** 2
                )
                - tar_rad
            )
            time = (
                2 * air_to_plastic / air_speed
