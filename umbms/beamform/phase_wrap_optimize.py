...
if __name__ == "__main__":
    ...
    mask = freqs > 3.5e9
    mse = {}
    titles = ['Wrapped Phase Initial Fit',
    ...
    mse['Unwrapped Phase Initial Fit'] =\
        ((1/np.size(shape)) *
    ...
    results_wrapped = minimize(fun=phase_diff_MSE,
    ...
    mse['Wrapped Phase on Wrapped Fit'] = ((1/np.size(shape)) *
                                           (np.sum((target_phase - shape)**2)))
    mse['Unwrapped Phase on Wrapped Fit'] =\
    ...
    for idx, axis in enumerate(ax.flat):
        axis.grid()
        axis.set_title(f'{titles[idx]} MSE = {mse[titles[idx]]:.3f}',
                       fontsize=10)
