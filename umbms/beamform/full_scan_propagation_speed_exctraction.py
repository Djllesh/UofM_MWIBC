...
freqs_2d = np.tile(
    freqs.reshape(np.size(target_phase, axis=0), 1),
...
if __name__ == "__main__":
    ...
    results = minimize(
        fun=phase_diff_MSE,
        x0=np.array([3.40, 17.93, 101.75, 0.18, -target_phase[0, 0]]),
    ...
    raw_speed_5pi = (
        ...
        * freqs_2d
        * length
        / (target_phase_unwrapped - 2 * np.pi * 5)
    )
    ...
    raw_speed_6pi = (
        ...
        * freqs_2d
        * length
        / (target_phase_unwrapped - 2 * np.pi * 6)
    )
    ...
    phase_speed_5pi_in = phantom_width / (
        length / phase_speed_5pi - (length - phantom_width) / 2.2e8
    )

    phase_speed_6pi_in = phantom_width / (
        length / phase_speed_6pi - (length - phantom_width) / 2.2e8
    )
