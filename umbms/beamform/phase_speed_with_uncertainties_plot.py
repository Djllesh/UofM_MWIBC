...
from umbms.beamform.phase_speed_uncertainties import dv_avg
...
if __name__ == "__main__":
    ...
    fit, error = fit_bootstrap_minimize(
        p0=p0,
        datax=freqs,
        datay=target_phase_unwrapped,
        function=phase_diff_MSE,
        length=length,
        resfunc=phase_shape_explicit,
    )
    ...
    raw_speed_5pi = (
        -2 * np.pi * freqs * length / (target_phase_unwrapped - 2 * np.pi * 5)
    )
    ...
    dv_avg_5_pi = dv_avg(
        v=phase_speed_5pi,
        phi=shape_unwrapped - 2 * 5 * np.pi,
        e_prime=np.real(epsilon),
        e_2prime=np.abs(np.imag(epsilon)),
        freqs=freqs,
        e_h=e_h,
        e_s=e_s,
        tau=tau,
