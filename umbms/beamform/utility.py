...
def get_xy_arrs(m_size, roi_rad):
    ...
    # X arrays are horizontal, Y arrays are vertical
    # (transposition discrepancy fixed)
    x_dists = np.linspace(-roi_rad, roi_rad, m_size)
...
def get_pixdist_ratio(m_size, roi_rad):
    ...
    pix_to_dist_ratio = m_size / (2 * roi_rad)
...
def get_ant_xy_idxs(ant_rad, roi_rad, n_ant_pos, m_size,
    ...
    ant_x_idxs = np.floor(ant_xs * pixdist_ratio).astype(int)
    ant_y_idxs = np.floor(ant_ys * pixdist_ratio).astype(int)
