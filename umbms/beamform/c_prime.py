...
if __name__ == "__main__":
    ...
    raw_speed_2pi = - 2 * np.pi * freqs * length / (target_phase_unwrapped -
                                                    2 * np.pi * 2)

    raw_speed_3pi = - 2 * np.pi * freqs * length / (target_phase_unwrapped -
                                                    2 * np.pi * 3)

    phase_speed_2pi = - 2 * np.pi * freqs * length / (shape_unwrapped - 2 *
                                                      np.pi * 2)

    phase_speed_3pi = - 2 * np.pi * freqs * length / (shape_unwrapped - 2 *
                                                      np.pi * 3)
    __MY_DPI = 120
    fig, ax = plt.subplots(**dict(figsize=(1500 / __MY_DPI, 720 / __MY_DPI),
                                  dpi=__MY_DPI))

    ax.plot(freqs, raw_speed_2pi, 'r--', label=r'Unfitted speed '
                                               r'$-2 \cdot 2\pi$',
            linewidth=0.7)
    ax.plot(freqs, raw_speed_3pi, 'b--', label=r'Unfitted speed '
                                               r'$-2 \cdot 3\pi$',
            linewidth=0.7)

    ax.plot(freqs, phase_speed_2pi, 'r--', label=r'Extracted speed, shift = '
                                                 r'$-2 \cdot 2\pi$',
            linewidth=1.2)
    ax.plot(freqs, phase_speed_3pi, 'b--', label=r'Extracted speed, shift = '
                                                 r'$-2 \cdot 3\pi$',
            linewidth=1.2)
