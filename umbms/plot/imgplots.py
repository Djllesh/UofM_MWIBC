...
def plot_img(
    ...
    if crop_img:
        ...
        in_ant_trajectory = breastmodels.get_roi(
    ...
    if normalize:  # If normalizing the image
        ...
        if max_val != 1.0:
            img_to_plt = img_to_plt - np.min(
                img_to_plt[np.logical_not(np.isnan(img_to_plt))]
            )
...
def plot_fd_img(
    ...
    *,
    ...
    if adi_rad == 0:
        breast_xs, breast_ys = (
            mid_breast_min * 100 * np.cos(draw_angs),
            mid_breast_max * 100 * np.sin(draw_angs),
        )
    else:
        breast_xs, breast_ys = (
            adi_rad * 100 * np.cos(draw_angs) + ox * 100,
    ...
    if partial_ant_idx is not None:
        ...
        ant_draw_angs = np.linspace(0, np.deg2rad(355), 72) + np.deg2rad(-136.0)
        # Make the marks inside the ROI
        new_ant_rad = roi_rad - 0.5
        ant_xs, ant_ys = (
            new_ant_rad * np.cos(ant_draw_angs),
    ...
    if loc_err:
        plt.plot(
            ...
            tum_ys_unc,
            "r--",
            label="Uncorrected reconstructed position",
            linewidth=1.3,
        )
        plt.plot(
            tum_xs_cor,
            tum_ys_cor,
            "b--",
            label="Corrected reconstructed position",
            linewidth=1.3,
        )

    tar2_xs, tar2_ys = (
        tar2_rad * 100 * np.cos(draw_angs) + tar2_x * 100,
...
def antennas_to_shifted_boundary(
    ...
    ant_angs = np.linspace(0, np.deg2rad(fin_ant_ang), n_ant_pos) + np.deg2rad(
        ini_ant_ang
    )
    ...
    for ant_pos in [4, 52]:
        ...
        closest_unshifted_idx = np.argmin(
            np.sqrt((unshifted_x - xs) ** 2 + (unshifted_y - ys) ** 2)
        )

        closest_unshifted_idxs = [
        ...
        shifted_idx = np.argmin(
            np.sqrt(
                (ant_xs[ant_pos] - plot_xs_shifted) ** 2
                + (ant_ys[ant_pos] - plot_ys_shifted) ** 2
            )
        )
        ...
        plt.plot(
            (ant_xs[ant_pos], plot_xs_shifted[shifted_idx]),
...
def calculate_arc_map_known_time(
    pix_ts, times_signals, *, n_ant_pos=72, threshold=0.5e-11
):
    arc_map = np.zeros_like(pix_ts)
...
def plot_arc_map(
    ...
    *,
    ...
    if arc_map is None:
        arc_map = calculate_arc_map(
            pix_ts, td_data, iczt_time, n_ant_pos=n_ant_pos, threshold=threshold
        )
    ...
    plt.gca().set_yticks([-6, -4, -2, 0, 2, 4, 6])
    plt.tight_layout()
    plt.savefig(save_str, transparent=transparent, dpi=300, bbox_inches="tight")

    plt.close()


def plot_known_arc_map(
    arc_map,
    img_roi,
    save_str,
    *,
