...
for i in range(2):
    for j in range(3):
        ...
        if i == 0 and j == 2:
            ...
            shift_step = (
                0.02  # Amount of shift to the top-right for each circle
            )
        ...
        if i == 1 and j == 1:
            ...
            for ii in range(n_circles):
                ...
                r_noise = add_perlin_noise_2d(
                    theta, noise_strength=noise_strength + 0.05, scale=3.0
                )
        ...
        r_noise = add_perlin_noise_2d(
            theta, noise_strength=radius_noise, scale=3.0
        )
...
r_noise_last = add_perlin_noise_2d(
    theta, noise_strength=noise_strength, scale=3.0
)
...
mask = get_binary_mask(
    cs_r, m_size=m_size, roi_rad=np.abs(xlim[0] - xlim[1]) / 2
)
...
im = ax_last.imshow(
    perlin_grid_normalized, extent=extent, cmap="viridis", origin="lower"
)
