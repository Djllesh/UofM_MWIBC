...
if __name__ == "__main__":
    ...
    ax[0, 0].plot(plot_freqs[mask], glycerin_fit[mask], 'r-',
                  label=r'Estimated speed inside, shift = $-2 \cdot 4\pi$',
                  linewidth=1.3)
    ax[0, 0].set_title('Glycerin', fontsize=16)

    ax[0, 1].plot(plot_freqs[mask], dgbe95_exp[mask], 'r--',
                  linewidth=0.9)
    ax[0, 1].plot(plot_freqs[mask], dgbe95_fit[mask], 'r-',
                  label=r'Estimated speed inside, shift = $-2 \cdot 4\pi$',
                  linewidth=1.3)
    ax[0, 1].set_title('DGBE 95%', fontsize=16)

    ax[1, 0].plot(plot_freqs[mask], dgbe90_exp[mask], 'r--',
                  linewidth=0.9)
    ax[1, 0].plot(plot_freqs[mask], dgbe90_fit[mask], 'r-',
                  label=r'Estimated speed inside, shift = $-2 \cdot 4\pi$',
                  linewidth=1.3)
    ax[1, 0].set_title('DGBE 90%', fontsize=16)

    ax[1, 1].plot(plot_freqs[mask], dgbe70_exp[mask], 'r--', label="Experimental phase",
                  linewidth=0.9)
    ax[1, 1].plot(plot_freqs[mask], dgbe70_fit[mask], 'r-',
