...
glycerin_data = 1e-8*np.genfromtxt('data/glycerin_full.csv', skip_header=1,
                              delimiter=',')
glycerin_exp = glycerin_data[:, 1]
glycerin_phase = glycerin_data[:, 2]

dgbe95_data = 1e-8*np.genfromtxt('data/dgbe95_full.csv', skip_header=1,
                            delimiter=',')
dgbe95_exp = dgbe95_data[:, 1]
dgbe95_phase = dgbe95_data[:, 2]

dgbe90_data = 1e-8*np.genfromtxt('data/dgbe90_full.csv', skip_header=1,
                            delimiter=',')
dgbe90_exp = dgbe90_data[:, 1]
dgbe90_phase = dgbe90_data[:, 2]

dgbe70_data = 1e-8*np.genfromtxt('data/dgbe70_full.csv', skip_header=1,
                            delimiter=',')
...
if __name__ == "__main__":
    ...
    ax[0, 0].plot(plot_freqs, glycerin_phase, 'r-',
            label=r'Estimated speed inside, shift = $-2 \cdot 4\pi$',
            linewidth=1.3)
    ax[0, 0].set_title('Glycerin', fontsize=16)

    ax[0, 1].plot(plot_freqs, dgbe95_exp, 'k-',
            linewidth=1.3)
    ax[0, 1].plot(plot_freqs, dgbe95_phase, 'r-',
            label=r'Estimated speed inside, shift = $-2 \cdot 4\pi$',
            linewidth=1.3)
    ax[0, 1].set_title('DGBE 95%', fontsize=16)

    ax[1, 0].plot(plot_freqs, dgbe90_exp, 'k-',
            linewidth=1.3)
    ax[1, 0].plot(plot_freqs, dgbe90_phase, 'r-',
            label=r'Estimated speed inside, shift = $-2 \cdot 4\pi$',
            linewidth=1.3)
    ax[1, 0].set_title('DGBE 90%', fontsize=16)

    ax[1, 1].plot(plot_freqs, dgbe70_exp, 'k-',
            linewidth=1.3)
    ax[1, 1].plot(plot_freqs, dgbe70_phase, 'r-',
            label=r'Estimated speed inside, shift = $-2 \cdot 4\pi$',
            linewidth=1.3)
