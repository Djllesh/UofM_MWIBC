...
MY_DPI = 120
plt.rcParams['animation.ffmpeg_path'] = 'C:/Program Files (' \
...
def calculate_speed(phase):
    return - 2 * np.pi * freqs * length / phase

initial_speed = calculate_speed(target_phase_unwrapped)

fig, (ax1, axsp) = plt.subplots(1, 2, figsize=(1800/__MY_DPI, 1000/__MY_DPI))
...
axsp.set_ylabel('Propagation speed, (m/s)')
axsp.grid('-', linewidth=0.6)
propagation_speed_plot = axsp.plot(freqs, initial_speed, 'k-', linewidth=1.4)

ax2 = ax1.twinx()
ax1.set_xlim(1.8e9, 9.2e9)
ax2.set_xlim(1.8e9, 9.2e9)
ax2.set_ylim(top=target_phase_unwrapped[0]+3,
             bottom=target_phase_unwrapped[-1]-23)
ax2.set_ylabel('Unwrapped phase, rad', color='black')
unwrapped_phase_plot = ax2.plot(freqs, target_phase_unwrapped, 'k-',
                                linewidth=1.4)

initial_phase = ax2.axhline(y=target_phase_unwrapped[0],
                            xmin=(2 - 1.8)/(9.8 - 1.8),
                            color='black', linestyle='-', linewidth=.9,
                            alpha=0.8)
point, = ax2.plot(freqs[0], target_phase_unwrapped[0], 'kx')

# Create an empty annotation box without an arrow
annotation = ax2.annotate(f'Shift: - {0:.2f} rad', xy=(9.2e9,
                                                       target_phase_unwrapped[0]),
...
def shift_line(shift):
    ...
    new_propagation_speed = calculate_speed(target_phase_unwrapped - shift)
    propagation_speed_plot[0].set_ydata(new_propagation_speed)
    axsp.set_ylim(np.min(new_propagation_speed) - 0.1e8,
                  np.max(new_propagation_speed) + 0.1e8)
    initial_phase.set_data([(2 - 1.8)/( 9.8 - 1.8), 1],
    ...
    annotation.xy = (9.2e9, target_phase_unwrapped[0] - shift)  # Point for the box
    ...
    point, annotation
    return unwrapped_phase_plot[0], propagation_speed_plot[0], initial_phase,

ani = animation.FuncAnimation(fig=fig, func=shift_line,
...
# plt.show()
ani.save('C:/Users/prikh/Desktop/ani.mp4', writer=FFwriter, dpi=__MY_DPI)
