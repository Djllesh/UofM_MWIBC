...
def plot_pos_errs(imgs, xs, ys, compare_xs, compare_ys,
    ...
    for ii in range(np.size(imgs, axis=0)):  # For each img
        ...
        phi_errs[ii] = np.rad2deg(np.arctan2(ys[ii], xs[ii])
    ...
    plt.tight_layout()
    plt.show()
    plt.savefig(os.path.join(out_dir, '%s_recon_vs_obs_pos.png' % save_str),
                dpi=300, transparent=True)
    plt.savefig(os.path.join(out_dir, 'NT_%s_recon_vs_obs_pos.png' % save_str),
                dpi=300, transparent=False)

    init_plt()  # Plot x errors
    plt.scatter(x=np.arange(len(x_errs)), y=x_errs,
                color='k', marker='o', label='x-differences')
    plt.xlabel('Experiment Number', fontsize=20)
    plt.ylabel('x-Position Difference (mm)', fontsize=20)
    plt.grid()
    plt.tight_layout()
    plt.show()
    plt.savefig(os.path.join(out_dir, '%s_x_diffs.png' % save_str),
                dpi=300, transparent=True)
    plt.savefig(os.path.join(out_dir, 'NT_%s_x_diffs.png' % save_str),
                dpi=300, transparent=False)

    init_plt()  # Plot y errors
    plt.scatter(x=np.arange(len(y_errs)), y=y_errs,
                color='b', marker='x', label='y-differences')
    plt.xlabel('Experiment Number', fontsize=20)
    plt.ylabel('y-Position Difference (mm)', fontsize=20)
    plt.grid()
    plt.tight_layout()
    plt.show()
    plt.savefig(os.path.join(out_dir, '%s_y_diffs.png' % save_str),
                dpi=300, transparent=True)
    plt.savefig(os.path.join(out_dir, 'NT_%s_y_diffs.png' % save_str),
                dpi=300, transparent=False)

    init_plt()  # Plot phi errors
    plt.scatter(x=np.arange(len(y_errs)), y=phi_errs,
                color='b', marker='x', label='y-differences')
    plt.xlabel('Experiment Number', fontsize=20)
...
def position_compare(syst_errs, xs, ys, max_xs, max_ys):
    ...
    pos_diff = np.sum(np.sqrt((cor_xs - max_xs)**2
                              + (cor_ys - max_ys)**2))
...
def find_systematic_errs(max_xs, max_ys, xs, ys):
    ...
    # positions
    opt_res = minimize(position_compare, x0=np.array([0, 0, 0]),
...
def do_pos_err_analysis(imgs, tar_xs, tar_ys, logger, roi_rad, o_dir_str='',
    ...
    if 'DAS' in save_str:
        print(save_str)
        if use_img_maxes:
            save_pickle((img_xs, img_ys),
                    os.path.join(__OUT_DIR, 'img_coords_MAX.pickle'))
        else:
            save_pickle((img_xs, img_ys),
                        os.path.join(__OUT_DIR, 'img_coords_CoM.pickle'))
        save_pickle((tar_xs, tar_ys),
                    os.path.join(__OUT_DIR, 'meas_coords.pickle'))
    # TEMPORARY ABOVE --------------------------------------------------
...
def plt_rand_pos_errs(c_xs, c_ys, xs, ys, o_dir_str, logger,
                      save_str, make_plts=True):
    ...
    spearman_val, pval = spearmanr(a=np.sqrt(c_xs**2 + c_ys**2),
                                   b=pos_errs)
    ...
    if make_plts:
        init_plt()
        plt.scatter(10 * np.sqrt(c_xs ** 2 + c_ys ** 2),
                    10 * pos_errs,
                    marker='o',
                    color=viridis(0),
                    )
        plt.xlabel(r'Polar Radius of Target $\mathdefault{\rho}$ (mm)',
                   fontsize=22)
        ...
        plt.savefig(os.path.join(out_dir,
                    ...
                    transparent=True)
        plt.savefig(os.path.join(out_dir,
                                 'NT_%s_posErr_vs_PolarRho.png' % save_str),
                    dpi=300,
                    transparent=False)

        # Plot x-error distribution...
        init_plt()
        sns.histplot(10 * (xs - c_xs), label='DAS',
                    color=viridis(0.0),
                    alpha=0.3,
                    kde=True)
        plt.axvline(x=10 * np.mean(xs - c_xs),
        ...
        plt.savefig(os.path.join(out_dir, '%s_xError_Distros.png' % save_str),
                    dpi=300,
                    transparent=True)
        plt.savefig(os.path.join(out_dir, 'NT_%s_xError_Distros.png' % save_str),
                    dpi=300,
                    transparent=False)

        # Plot y-error distribution below...
        init_plt()
        sns.histplot(10 * (ys - c_ys), label='DAS',
                    color=viridis(0.0),
                    alpha=0.3,
                    kde=True)
        plt.axvline(x=10 * np.mean(ys - c_ys),
                    color=viridis(0), linestyle='--')
        ...
        plt.savefig(os.path.join(out_dir, '%s_yError_Distros.png' % save_str),
                    dpi=300,
                    transparent=True)
        plt.savefig(os.path.join(out_dir, 'NT_%s_yError_Distros.png' % save_str),
                    dpi=300,
                    transparent=False)

        init_plt()
        sns.histplot(10 * pos_errs, label='DAS',
                    color=viridis(0.0),
                    alpha=0.3,
                    kde=True)
        ...
        plt.savefig(os.path.join(out_dir, '%s_posError_Distros.png' % save_str),
                    ...
                    transparent=True)
        plt.savefig(os.path.join(out_dir,
                                 'NT_%s_posError_Distros.png' % save_str),
                    dpi=300,
                    transparent=False)
