...
if __name__ == "__main__":
    ...
    out_dir = os.path.join(
        __OUT_DIR, "recons/Immediate reference/20240109_glass_rod/ft_das/"
    )
    verify_path(out_dir)
    ...
    for expt in range(n_expts):  # for all scans
        ...
        if ~np.isnan(tar_md["emp_ref_id"]) and ~np.isnan(tar_md["adi_ref_id2"]):
            ...
            logger.info("\tReconstruction (DMAS)...")

            das_binary_recon = fd_dmas(
                fd_data=adi_cal_cropped, pix_ts=pix_ts, freqs=freqs
            )
            ...
            plot_fd_img(
                ...
                title="",
                save_fig=True,
                save_str=os.path.join(out_dir, "id_%d_dmas_binary.png" % expt),
                save_close=True,
            )
