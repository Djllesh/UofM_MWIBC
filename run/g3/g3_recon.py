    ...
    for fibr_perc in [0]:
        ...
        if "F" in tar_md["phant_id"]:
            ...
            expt_adi_out_dir = os.path.join(
                out_dir, "id-%d-adi-%.1f-fibr-perc/" % (tar_md["id"], fibr_perc)
            )
            ...
            expt_fib_out_dir = os.path.join(
                out_dir, "id-%d-fib-%.1f-fibr-perc/" % (tar_md["id"], fibr_perc)
            )
            ...
            ant_xs, ant_ys = get_ant_scan_xys(
                ant_rad=ant_rad, n_ant_pos=72, ini_ant_ang=-136.0
            )
            pix_xs, pix_ys = get_xy_arrs(
                m_size=__M_SIZE, ant_rad=roi_rad + 0.01
            )

            int_f_xs, int_f_ys, int_b_xs, int_b_ys = find_xy_ant_bound_ellipse(
                ant_xs[0],
                ant_ys[0],
                pix_xs[0, :],
                pix_ys[:, 0],
            ...
            dmas_adi_recon = fd_dmas(
                fd_data=adi_cal_cropped, pix_ts=pix_ts, freqs=scan_fs[tar_fs]
            )
            ...
            if ~np.isnan(tar_md["tum_diam"]):
                ...
                das_fib_recon = fd_das(
                    fd_data=fib_cal_cropped,
                    phase_fac=phase_fac,
                    freqs=scan_fs[tar_fs],
                    worker_pool=worker_pool,
                )
