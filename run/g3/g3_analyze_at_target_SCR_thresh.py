...
if __name__ == "__main__":
    ...
    for ii in range(n_expts):
        ...
        # If the experiment was of a healthy phantom
            ...
            # Load the DAS and DMAS reconstructions
            das_img = load_pickle(os.path.join(tar_img_dir, "das_adi.pickle"))
            dmas_img = load_pickle(os.path.join(tar_img_dir, "dmas_adi.pickle"))
    ...
    save_pickle(
        (das_sensitivity, dmas_sensitivity, orr_sensitivity),
        os.path.join(
            __OUT_DIR,
            "%s_sensitivities_at_target_threshold.pickle" % __TUM_REF_STR,
        ),
    )
    save_pickle(
        (das_specificity, dmas_specificity, orr_specificity),
        os.path.join(
            __OUT_DIR,
            "%s_specificities_at_target_threshold.pickle" % __TUM_REF_STR,
        ),
    )
    ...
    save_pickle(
        (das_tum_sens, dmas_tum_sens, orr_tum_sens),
        os.path.join(__OUT_DIR, "%s_sens_by_tums.pickle" % __TUM_REF_STR),
    )
