...
if __name__ == "__main__":
    ...
    out_dir = os.path.join(__OUT_DIR,
                           'recons/boundary_test/Sino/Correlation '
                           'step-by-step/')
    verify_path(out_dir)
    ...
    for ii in [131]:
        ...
        if 'F' in tar_md['phant_id']:
            ...
            cs, x_cm, y_cm = \
                get_boundary_iczt(adi_cal_cropped, ant_rad, ini_ant_ang=-136.)
            ...
            das_adi_recon = fd_das(fd_data=adi_cal_cropped_non_emp,
                                   phase_fac=phase_fac,
            ...
            plot_fd_img(img=np.abs(das_adi_recon),
                        cs=cs, tum_x=tum_x, tum_y=tum_y,
                        tum_rad=tum_rad, mid_breast_max=mid_breast_max,
                        mid_breast_min=mid_breast_min, ox=x_cm, oy=y_cm,
                        ant_rad=ant_rad, roi_rad=roi_rad,
                        img_rad=roi_rad, phantom_id=tar_md['phant_id'],
                        title='%s' % plt_str,
                        save_fig=True,
                        save_str=os.path.join(expt_adi_out_dir,
                                              'id_%d_adi_cal_das.png'
                                              % tar_md['id']),
                        save_close=True)

            bound_x, bound_y = find_boundary(np.abs(das_adi_recon), roi_rad,
                                             n_slices=110)
            ...
            for stl_z in np.arange(2, 7, 0.05):
                ...
                plt.legend()
                plt.savefig(os.path.join(expt_adi_out_dir,
                                         'id_%d_rho-phi_diagram_%.2f_z_slice'
                                         '.png '
                                         % (tar_md['id'], stl_z)), dpi=300)
                plt.close()
            ...
            mse_stats_dict[tar_md['phant_id'].split('F')[0]][
                'mse'] = np.append(
            ...
            print('Smallest MSE = %f, z_slice = %.2f' % (np.min(mse_array),
                                                         np.arange(2, 7, 0.05)[
                                                             np.argmin(
                                                                 mse_array)]))

    for shell in ['A2', 'A3', 'A14', 'A16']:
        print('Average MSE for shell %s = %f'
              % (shell, np.average(mse_stats_dict[shell]['mse'])))
        print('Average z_slice = %.2f'
