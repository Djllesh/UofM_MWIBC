...
from umbms.analysis.acc_poserr import (
    do_pos_err_analysis,
    apply_syst_cor,
    plt_rand_pos_errs,
    plot_obs_vs_recon,
...
if __name__ == "__main__":
    ...
    xs_antenna, ys_antenna = apply_syst_cor(
        xs=t_coords[:, 0],
        ys=t_coords[:, 1],
        x_err=-0.028,
        y_err=-0.027,
    ...
    scr_values_reg = np.zeros(
        [
            np.size(imgs_reg, axis=0),
        ]
    )
    smr_values_reg = np.zeros(
        [
            np.size(imgs_reg, axis=0),
        ]
    )
    ...
    for jj in range(np.size(imgs_reg, axis=0)):  # For every image
        ...
        rho_size, phi_size = do_size_analysis(
            img_here=np.abs(imgs_reg[jj, :, :]) ** 2,
            dx=dx,
            roi_rad=roi_rad,
            make_plts=False,
        )
    ...
    init_plt()
    plt.scatter(
        np.sqrt(t_coords[:, 0] ** 2 + t_coords[:, 1] ** 2),
        rho_sizes,
        color=colors[0],
    )
    ...
    scr_values_b = np.zeros(
        [
            np.size(imgs_b, axis=0),
        ]
    )
    smr_values_b = np.zeros(
        [
            np.size(imgs_b, axis=0),
        ]
    )
    ...
    scr_values_fdnc = np.zeros(
        [
            np.size(imgs_fdnc, axis=0),
        ]
    )
    smr_values_fdnc = np.zeros(
        [
            np.size(imgs_fdnc, axis=0),
        ]
    )
    ...
    for jj in range(np.size(imgs_fdnc, axis=0)):
        rho_size, phi_size = do_size_analysis(
            img_here=np.abs(imgs_fdnc[jj, :, :]) ** 2,
            dx=dx,
            roi_rad=roi_rad,
            make_plts=False,
        )
    ...
    scr_values_fd = np.zeros(
        [
            np.size(imgs_fd, axis=0),
        ]
    )
    smr_values_fd = np.zeros(
        [
            np.size(imgs_fd, axis=0),
        ]
    )
    ...
    scr_values_rt = np.zeros(
        [
            np.size(imgs_rt, axis=0),
        ]
    )
    smr_values_rt = np.zeros(
        [
            np.size(imgs_rt, axis=0),
        ]
    )
